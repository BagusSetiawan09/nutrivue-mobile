...
export default function ForgotPasswordScreen({ navigation }: any) {
  ...
  const handleEmailChange = (text: string) => {
    ...
    if (!emailRegex.test(text)) { setEmailError('Format email masih belum lengkap'); return; }
    const domain = text.split('@')[1]?.toLowerCase();
    const commonTypos = ['gamail.com', 'gmil.com', 'gmai.com', 'yaho.com'];
    if (commonTypos.includes(domain)) { setEmailError(`Sepertinya ada kesalahan. Apakah maksud Anda @${domain.startsWith('y') ? 'yahoo' : 'gmail'}.com?`); return; }
    setEmailError('');
  };
  ...
  const renderStep1 = () => (
    <View className="mt-6 w-full">
      ...
      <View className="mt-10">
        <TouchableOpacity disabled={!isEmailValid} onPress={() => setStep(2)} className={`${isEmailValid ? 'bg-primary' : 'bg-gray-300'} rounded-2xl py-4 shadow-sm items-center justify-center`}>
    ...
    <View className="mt-6 w-full">
      ...
      <View className="items-center mt-4">
        <TextInput 
          ...
          onChangeText={setOtp} 
        />
        <TouchableOpacity className="mt-6"><Text className="text-primary font-medium text-base">Kirim Ulang Kode (00:30)</Text></TouchableOpacity>
      </View>
      <View className="mt-10">
        <TouchableOpacity disabled={otp.length < 4} onPress={() => setStep(3)} className={`${otp.length === 4 ? 'bg-primary' : 'bg-gray-300'} rounded-2xl py-4 shadow-sm items-center justify-center`}>
  ...
  const renderStep3 = () => (
    <View className="mt-6 w-full">
      <View className="mb-10">
        ...
        <Text className="text-base text-gray-500 leading-relaxed">Sandi Anda telah diverifikasi. Silakan buat kata sandi baru yang kuat.</Text>
      </View>
      <View className="space-y-5">
        <View>
          ...
          <View className="relative justify-center">
            ...
            />
            <TouchableOpacity className="absolute right-4 p-1" onPress={() => setShowPassword(!showPassword)}><Ionicons name={showPassword ? "eye-outline" : "eye-off-outline"} size={22} color="#9CA3AF" /></TouchableOpacity>
          </View>
        </View>
        <View>
          ...
          <View className="relative justify-center">
            <TextInput 
              className={`bg-white border ${confirmPassword.length > 0 && !isPasswordMatch ? 'border-red-500' : 'border-gray-100'} rounded-2xl pl-5 pr-12 py-4 text-base text-gray-900 shadow-sm`} 
            ...
            />
            <TouchableOpacity className="absolute right-4 p-1" onPress={() => setShowConfirmPassword(!showConfirmPassword)}><Ionicons name={showConfirmPassword ? "eye-outline" : "eye-off-outline"} size={22} color="#9CA3AF" /></TouchableOpacity>
          </View>
      ...
      <View className="mt-10">
        <TouchableOpacity disabled={!isPasswordMatch} onPress={() => navigation.navigate('Login')} className={`${isPasswordMatch ? 'bg-primary' : 'bg-gray-300'} rounded-2xl py-4 shadow-sm items-center justify-center`}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        ...
        >
          <View className="flex-1 px-8 py-10">
            <TouchableOpacity 
              onPress={() => { if (step === 1) navigation.goBack(); if (step === 2) setStep(1); if (step === 3) setStep(2); }} 
              className="mb-4 w-10"
            >
